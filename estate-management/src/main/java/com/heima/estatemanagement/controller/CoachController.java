...
public class CoachController {
    ...
    public PageResult search(@RequestBody Map searchMap){
        Page<Coach> page = coachService.search(searchMap);
        return new PageResult(true, StatusCode.OK, MessageConstant.COACH_SEARCH_SUCCESS,page.getResult(),page.getTotal());
    }

    @RequestMapping("/add")
    public Result add(@RequestBody Coach coach){
        Boolean add = coachService.add(coach);
        return new Result(true,StatusCode.OK,MessageConstant.COACH_ADD_SUCCESS);
    }
    @RequestMapping("/findById")
    public Result findById(Integer id){
        Coach coach = coachService.findById(id);
        return new Result(true,StatusCode.OK,MessageConstant.COACH_FIND_BY_ID_SUCCESS,coach);
    }

    @RequestMapping("/update")
    public Result update(@RequestBody Coach coach){
        Boolean add = coachService.update(coach);
        return new Result(true,StatusCode.OK,MessageConstant.COACH_UPDATE_SUCCESS);
    }

    @RequestMapping("/del")
    public Result del(@RequestBody List<Integer> ids){
        Boolean flag = coachService.del(ids);
        return new Result(true,StatusCode.OK,MessageConstant.COACH_DELETE_SUCCESS);
    }
}
